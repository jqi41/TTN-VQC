...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        test_loss_laplace = 0
        with torch.no_grad():
            for idx in range(len(test_normal_data)):
                ...
                test_loss_normal += F.l1_loss(output_normal, target, reduction='sum').item()
                test_loss_laplace += F.l1_loss(output_laplace, target, reduction='sum').item()
           #     pred = output.argmax(dim=1, keepdim=True)
           #     correct += pred.eq(target.view_as(pred)).sum().item()
            test_loss_normal /= len(test_normal_data)  
            test_loss_laplace /= len(test_laplace_data)
